...
print("Checking that the number of rows and columns are the same for each matrix")
# if(all.equal(lengths(lapply(matrices, nrow)),lengths(lapply(matrices, ncol)))){
#     print("All matrices have the same number of rows and columns")
# } else {
...
qs::qsave(rse_list, file = OUTPUT$CNV_se, nthreads = THREADS)
