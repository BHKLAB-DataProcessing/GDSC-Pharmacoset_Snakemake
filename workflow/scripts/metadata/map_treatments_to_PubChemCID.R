...
str(pubchem_annotated)


treatmentMetadata_pubchem_annotated <- merge(
    treatmentMetadata,
    pubchem_annotated,
    by = "GDSC.treatmentid",
    all.x = TRUE
...
treatmentMetadata_pubchem_annotated[, treatmentid := GDSC.treatmentid]
message("Writing to ", OUTPUT$treatment_CIDS)
data.table::fwrite(
    treatmentMetadata_pubchem_annotated, 
    OUTPUT$treatment_CIDS, 
    quote = FALSE, 
    sep = "\t", 
    row.names = FALSE
)
